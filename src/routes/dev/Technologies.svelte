<script lang="ts">
	import { Tabs, TabsList, TabsTrigger, TabsContent } from '$lib/components/ui/tabs/index.js';
	import BadgeList from './BadgeList.svelte';

	const languages: App.Badge[] = [
		{ name: 'Typescript', slug: 'Typescript', loved: true, color: '3178C6' },
		{ name: 'Javascript', slug: 'Javascript', logoColor: '333333', color: 'F7DF1E' },
		{ name: 'Julia', slug: 'Julia', loved: true, color: '9558B2' },
		{
			name: 'Java',
			slug: `data:image/svg%2bxml;base64,`,
			color: '007396'
		},
		{ name: 'C++', slug: 'cplusplus', loved: true, color: '00599C' },
		{ name: 'C#', slug: 'csharp', color: '239120' },
		{ name: 'Python', slug: 'Python', color: '3776AB' }
	];

	const frontend: App.Badge[] = [
		{ name: 'SvelteKit', slug: 'Svelte', loved: true, color: 'FF3E00' },
		{ name: 'React', slug: 'React', logoColor: '333333', color: '61DAFB' },
		{ name: 'HTML', slug: 'HTML5', color: 'E34F26' },
		{ name: 'CSS', slug: 'CSS3', color: '1572B6' },
		{ name: 'SCSS', slug: 'SASS', color: 'CC6699' },
		{ name: 'jQuery', slug: 'jquery', color: '0769AD' },
		{ name: 'Tailwind', slug: 'tailwindcss', loved: true, color: '06B6D4' },
		{ name: 'Shadcn UI', slug: 'shadcnui', loved: true, color: '000000' },
		{ name: 'Material Design', slug: 'materialdesign', color: '757575' },
		{ name: 'Carbon Design', slug: 'ibm', color: '052FAD' },
		{ name: 'Markdown', slug: 'markdown', loved: true, color: '000000' }
	];

	const tools: App.Badge[] = [
		{ name: 'Figma', slug: 'figma', loved: true, color: 'F24E1E' },
		{ name: 'Git', slug: 'git', loved: true, color: 'F05032' },
		{ name: 'VSCode', slug: 'visualstudiocode', loved: true, color: '007ACC' },
		{ name: 'Sublime Text', slug: 'sublimetext', color: 'FF9800' },
		{ name: 'Neovim', slug: 'neovim', color: '57A143' },
		{ name: 'Jupyter', slug: 'jupyter', loved: true, color: 'F37626' },
		{ name: 'Powershell', slug: 'powershell', color: '5391FE' },
		{ name: 'Firefox', slug: 'firefoxbrowser', loved: true, color: 'FF7139' },
		{ name: 'Chrome', slug: 'google chrome', color: '4285F4' },
		{ name: 'Obsidian', slug: 'obsidian', loved: true, color: '483699' },
		{ name: 'Thunderbird', slug: 'thunderbird', loved: true, color: '0A84FF' }
	];

	const backend: App.Badge[] = [
		{ name: 'Node.js', slug: 'node.js', loved: true, color: '339933' },
		{ name: '.NET', slug: 'dotnet', color: '512BD4' },
		{ name: 'Express', slug: 'express', color: '000000' },
		{ name: 'Socket.io', slug: 'socket.io', loved: true, color: '010101' },
		{ name: 'Auth0', slug: 'auth0', loved: true, color: 'EB5424' },
		{ name: 'Docker', slug: 'docker', loved: true, color: '2496ED' },
		{ name: 'Linux', slug: 'linux', loved: true, logoColor: '333333', color: 'FCC624' },
		{ name: 'NGINX', slug: 'nginx', loved: true, color: '009639' },
		{ name: 'Pocketbase', slug: 'pocketbase', loved: true, logoColor: '333333', color: 'B8DBE4' },
		{ name: 'Supabase', slug: 'supabase', loved: true, color: '3ECF8E' },
		{ name: 'Firebase', slug: 'firebase', logoColor: '333333', color: 'FFCA28' }
	];

	const database: App.Badge[] = [
		{ name: 'Postgres', slug: 'postgresql', loved: true, color: '4169E1' },
		{ name: 'SQLite', slug: 'sqlite', color: '003B57' },
		{ name: 'Redis', slug: 'redis', loved: true, color: 'DC382D' },
		{ name: 'MongoDB', slug: 'mongodb', color: '47A248' }
	];

	const hosting: App.Badge[] = [
		{ name: 'Cloudflare Pages', slug: 'cloudflarepages', loved: true, color: 'F38020' },
		{ name: 'Vercel', slug: 'vercel', color: '000000' },
		{ name: 'Netlify', slug: 'netlify', color: '00C7B7' },
		{ name: 'Vultr', slug: 'vultr', color: '007BFC' },
		{ name: 'Hetzner', slug: 'hetzner', loved: true, color: 'D50C2D' },
		{ name: 'Zoho', slug: 'zoho', color: 'C8202B' },
		{ name: 'Heroku', slug: 'heroku', color: '430098' },
		{ name: 'Render', slug: 'render', color: '46E3B7' }
	];

	const cloud: App.Badge[] = [
		{ name: 'Cloudflare', slug: 'cloudflare', loved: true, color: 'F38020' },
		{ name: 'Github', slug: 'github', loved: true, color: '181717' },
		{ name: 'AWS', slug: 'amazonaws', color: '232F3E' },
		{ name: 'Google Cloud', slug: 'googlecloud', color: '4285F4' },
		{ name: 'Azure', slug: 'microsoftazure', color: '0078D4' }
	];

	const graphics: App.Badge[] = [
		{ name: 'OpenGL', slug: 'opengl', loved: true, color: '5586A4' },
		{ name: 'WebGL', slug: 'webgl', loved: true, color: '990000' },
		{ name: 'p5.js', slug: 'p5.js', color: 'ED225D' }
	];

	const mlData: App.Badge[] = [
		{ name: 'OpenCV', slug: 'opencv', color: '5C3EE8' },
		{ name: 'Jupyter', slug: 'jupyter', color: 'F37626' },
		{ name: 'NumPy', slug: 'numpy', loved: true, color: '013243' },
		{ name: 'Pandas', slug: 'pandas', color: '150458' },
		{ name: 'Pytorch', slug: 'pytorch', loved: true, color: 'EE4C2C' },
		{ name: 'Tensorflow', slug: 'tensorflow', color: 'FF6F00' },
		{ name: 'sklearn', slug: 'scikit-learn', color: 'F7931E' },
		{ name: 'Optuna', slug: 'optuna', color: '002C76' }
	];

	const tabs = [
		{ value: 'languages', label: 'Languages', badges: languages },
		{ value: 'data', label: 'Data / ML', badges: mlData },
		{ value: 'frontend', label: 'Frontend', badges: frontend },
		{ value: 'tools', label: 'Tools', badges: tools },
		{ value: 'backend', label: 'Backend', badges: backend },
		{ value: 'db', label: 'DB', badges: database },
		{ value: 'hosting', label: 'Hosting', badges: hosting },
		{ value: 'cloud', label: 'Cloud', badges: cloud },
		{ value: 'graphics', label: 'Graphics', badges: graphics }
	];
</script>

<Tabs value="tools">
	<TabsList class="flex-wrap h-auto gap-1 mb-4">
		{#each tabs as { value, label }}
			<TabsTrigger {value}>{label}</TabsTrigger>
		{/each}
	</TabsList>
	{#each tabs as { value, badges }}
		<TabsContent {value}>
			<div class="flex flex-wrap gap-y-2 gap-x-2 w-full">
				<BadgeList {badges} />
			</div>
		</TabsContent>
	{/each}
</Tabs>
