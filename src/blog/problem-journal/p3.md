...
We can try to a different way of resolving $\text{count }{k}$. Consider the Sylow $p$-subgroups $\text{Syl}_p$ (each of which is characteristic) of $(\mathbb{Z}/m \mathbb{Z})^{\times}$, which is finite abelian:

$$
(\mathbb{Z}/m \mathbb{Z})^{\times} \cong \bigoplus_{p} {\text{Syl}_p}
$$

Moreover, if $p \mid \text{ord } g$ if and only if the component of $\text{Syl}_{p}$ is nonzero. These facts are easily seen from the primary decomposition of modules by viewing $(\mathbb{Z}/m \mathbb{Z})^{\times}$ as a torsion $\mathbb{Z}$-module. This is extremely useful here, because we now note that we can instead consider the count over subsets of $k = \prod q_j$ for $\phi(m) = \prod q_i^{\beta_i}$. 

$$
\sum_{d \mid k} \text{count }d = \left| \bigoplus \text{Syl}_{q_j} \right| = \prod \left| \text{Syl}_{q_j} \right| = \prod q_j^{\beta_j}
$$
